...
「そもそも未来を*描く*必要があるのか」→「出来上がった world model を何で測るのか」(W33) と降り、
**2026-W34 で分類マップの軸がついに完成した** —— しかも単なる分類ではなく **順序尺度**として。
**world model をいつ・どう使うかは「学習の誤りが決定にどれだけ直接効くか」の順に一列に並ぶ**:
**学習時 → 評価時 → 推論時(順位付け → 選択 → 探索規則)。** そして**受理基準の軸にも中身が入った** ——
**その world model を信じてよい根拠は再構成誤差でも probe 精度でもなく「潜在コストが候補を実タスクの順に並べるか」である。**
統合の場所も1つ増えた: **重みの内側だけでなく、凍結した基盤を包む実行時の外殻という選択肢がある。**
