...
削り方は5系統 — 学習し直さずに削る (schedule) / 学習して削る (one-step drift) / 生成を1回にする /
**生成を完走しない**(深さ方向の early exit + 出力段の除去、2026-W32) / そして
**2026-W33 で加わった「削るのではなく、そもそも本番に載せない」**(学習時だけ生成し、推論では枝ごと捨てる)。
最新の系統が示したのは、**削る対象は反復回数でも深さでもなく「経路そのもの」でありうる**こと、そして
**捨てても壊れないことは、根性ではなく構造(参照関係の禁止)で保証できる**こと。
別方向の圧縮も1つ増えた —— **遷移を学習せず積分として書く**と、パラメータ・速度・外挿性能が同時に改善する。
